...
public class SentRepository {
    private static final String TAG = "SentRepository";
    private static final String PREF_NAME = "auth";

    private final SentRetrofitApi api;
    ...
    private final MutableLiveData<List<Email>> sentEmails = new MutableLiveData<>();

    // Retrofit API definition
    private interface SentRetrofitApi {
        @GET("mails/sent")
        Call<List<Email>> getSentEmails(@Header("Authorization") String authToken);

        @DELETE("sent/{id}")
        Call<Void> deleteFromSent(@Path("id") String emailId, @Header("Authorization") String authToken);
    ...
    public SentRepository(Context context) {
        this.context = context;
        this.executor = Executors.newFixedThreadPool(2);
        this.mainHandler = new Handler(Looper.getMainLooper());

        // Initialize with empty list
        this.sentEmails.setValue(Collections.emptyList());
        ...
        this.api = retrofit.create(SentRetrofitApi.class);
    ...
    public void loadSentEmails() {
        ...
        api.getSentEmails(authToken).enqueue(new RetrofitCallback<List<Email>>() {
            @Override
            public void onResponse(Call<List<Email>> call, Response<List<Email>> response) {
                isLoading.postValue(false);
                if (response.isSuccessful() && response.body() != null) {
                    Log.d(TAG, "Loaded " + response.body().size() + " sent emails from API");
                    sentEmails.postValue( response.body());
                } else {
                    Log.e(TAG, "Failed to load sent emails: code " + response.code());
                    errorMessage.postValue("Failed to load sent emails: " + response.code());
                }
            }
            ...
            public void onFailure(Call<List<Email>> call, Throwable t) {
                isLoading.postValue(false);
                Log.e(TAG, "Error loading sent emails", t);
                errorMessage.postValue("Failed to load sent emails: " + t.getMessage());
            }
        });
    }

    public void deleteFromSent(Email email) {
        api.deleteFromSent(email.getId(), authToken).enqueue(new RetrofitCallback<Void>() {
            @Override
            public void onResponse(Call<Void> call, Response<Void> response) {
                if (response.isSuccessful()) {
                    Log.d(TAG, "Deleted email from sent: " + email.getId());
