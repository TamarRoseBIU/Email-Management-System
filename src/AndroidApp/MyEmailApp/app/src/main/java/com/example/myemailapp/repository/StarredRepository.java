...
public class StarredRepository {
    private static final String TAG = "StarredRepository";
    private static final String PREF_NAME = "auth";

    private final StarredRetrofitApi api;
    ...
    private final MutableLiveData<List<Email>> starredEmails = new MutableLiveData<>();

    // Retrofit API definition
    private interface StarredRetrofitApi {
        @GET("starred")
        Call<List<Email>> getStarredEmails(@Header("Authorization") String authToken);
    }
    ...
    public StarredRepository(Context context) {
        ...
        this.starredEmails.setValue(Collections.emptyList());
        ...
        this.api = retrofit.create(StarredRetrofitApi.class);
    ...
    public void loadStarredEmails() {
        ...
        api.getStarredEmails(authToken).enqueue(new RetrofitCallback<List<Email>>() {
            @Override
            public void onResponse(Call<List<Email>> call, Response<List<Email>> response) {
                isLoading.postValue(false);
                if (response.isSuccessful() && response.body() != null) {
                    Log.d(TAG, "Loaded " + response.body().size() + " starred emails from API");
                    starredEmails.postValue( response.body());
                } else {
                    Log.e(TAG, "Failed to load starred emails: code " + response.code());
                    errorMessage.postValue("Failed to load starred emails: " + response.code());
                }
            }
            ...
            public void onFailure(Call<List<Email>> call, Throwable t) {
                isLoading.postValue(false);
                Log.e(TAG, "Error loading starred emails", t);
                errorMessage.postValue("Failed to load starred emails: " + t.getMessage());
            }
        });
    }
